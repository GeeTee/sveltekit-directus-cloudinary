<script>
    ...
    // let gRecpatchaError = ""
    // console.log('contactForm expected', {recaptcha}, {smtpToken})
    // console.log('contactForm expected recaptcha', recaptcha)
    ...
    $: subjectValid = checkEntry(subject, 'subject') ? true : false
    ...
    // $: captchaValid = submitUserForm()
    let captchaValid = true

    $: formValid = lastNameValid && firstNameValid && subjectValid && messageValid && phoneValid && emailValid && rgpdValid && captchaValid

    // $: console.log('test Rgpd', checkRgpd(checked))

    let showHelpLN = false
    let showHelpFN = false
    let showHelpSubj = false
...
<form id="contact-form" name="contact">
    <legend class="subtitle is-4 is-uppercase">Adresser un mail à {siteName}</legend>
    <div class="columns">
    ...
    <div class="field">
        <label for="subject" class="label"><span class="has-text-danger">*</span> Sujet</label>
        <div class="control has-icons-left">
            <input 
            id="subject" 
            bind:value="{subject}"
            class="input" 
            class:is-danger={showHelpSubj}
            class:is-success={subjectValid}
            on:blur={() => {showHelpSubj = toggleHelp(subject, 'subject')}}
            type="text" 
            name="subject" 
            required
            >
        ...
        <p class="help has-text-danger" class:is-invisible={!showHelpSubj}>Champ indispensable (un certain nombre de caractères ne pourront pas être utilisés ex  {@html '>, <, [, ])'}</p>
    </div>
